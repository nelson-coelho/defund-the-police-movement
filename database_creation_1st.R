...
#              used in the article. We begin by loading files
#              for the land area of cities and pullind data from 
#              the CENSUS ACS API, followed by 
#              retrieving data for arrests and personnel 
#              from the FBI API Crime Explorer and LEOKA data.
#
# Modifications:
#
# Changes:
...
# Loading Kaplan's db
# Following is performed to retrieve ORI values to get data from FBI Crime Explorer for arrests and police personnel
shr_db <- read_dta('C:/shr_1976_2022.dta') # Check for the correct file and path

# Keeping years from 2010-2022
shr_db <- shr_db %>%  filter(shr_db$year>='2010')

# Doing below to fix the oris
...
for (i in 1:3768) {
  ...
  if (length(Arrest_db1) != 0) {
    ...
    #only runs if statement is the first iteration of for loop, then runs alternative for every other iteration
    if (i==1) {
...
#saving just in case
write.csv(Arrest_db, "C:/Arrest_db.csv", row.names=FALSE)
...
for (i in 2010:2022) {
  randname <- get_acs(
    ...
    year = i,
    output = "wide",
    survey = "acs5") #acs 5 years
  randname$year = i
  CENSUS2010_2022PULL <- rbind(CENSUS2010_2022PULL, randname) #bind to database
}

#saving just in case
write.csv(CENSUS2010_2022PULL, "C:/CENSUS2010_2022PULL.csv", row.names=FALSE)
...
#saving just in case
write.csv(land_2010_2022, "C:/land_2010_2022.csv", row.names=FALSE)
...
for (i in 2010:2022) {
  ...
  print(count) # validation check
    ...
    leoka <- read_dta(paste0("C:/leoka_monthly_"
                              , as.character(i)
                              ,".dta"))
...
#saving just in case
write.csv(leoka_2010_2022, "C:/leoka_2010_2022.csv", row.names=FALSE)
...
ori_db <- subset(ori_db, select = c(ori, year)) #keep only needed vars
...
#saving just in case
write.csv(oriyear, "C:/main_initial_db.csv", row.names=FALSE)

#SAVING NEW SHR
#write.csv(shr_db, "C:/shr_db_filtered.csv", row.names=FALSE)


#===============================================================================
# Load and prepare the data ====================================================
# Loading CENSUS db
CENSUS2010_2022 <- read.csv('C:/CENSUS2010_2022PULL.csv')
# Loading land area data
land_2010_2022 <- read.csv('C:/land_2010_2022.csv')
...
#saving
#write.csv(ACS, "C:/ACS.csv", row.names=FALSE)
