...
# Version: Article v1
#
# Purpose: This code performs the imputation of 0 for NA and subsequently MICE RF.
#
...
# Date: 2024-11-27
#
# File Name: imputation_0_for_NA_6.1.R
#
# Description: This file contains R code to impute 0 for NA because SHR
#              table is a table of occurrences, then another imputation of MICE RF.
#
# Modifications:
#
# Changes:
...
#===============================================================================
###########################################
# below is the 0 imputation performed     #
#  only for variables that come from      #  
# LEOKA and SHR databases (occurrences)   #
###########################################
#===============================================================================
# we perform a validation first
# if city has data for arrests on FBI API database, then we imput 0 on every NA
# of these vars... otherwise we assume that city truly did not send data to FBI
# and it remains NA
#
# the following variables undergo this imputation:
#
#
# homs_occurrence = occurrence of homicides
# pol_JHs_occurrence  = occurrence of police justifiable homicides
# cit_JHs_occurrence  = occurrence of citizen justifiable homicides
# leoka_killed = occurrence of officers killed
# leoka_assaulted = occurrence of officers assaulted
#===============================================================================

for (i in 1:nrow(dtp_db_v3_1)) {
  row <- dtp_db_v3_1[i,]
  if (!is.na(row$arrests) & is.na(row$homs_occurrence)) {
    row$homs_occurrence = 0
    row$homs_additional_victim_count = 0
...
#save copy
#write.csv(dtp_db_v3_1, "C:/dtp_db_v3_1_1.csv", row.names=FALSE)


for (i in 1:nrow(dtp_db_v3_1)) {
  row <- dtp_db_v3_1[i,]
  if (!is.na(row$arrests) & is.na(row$pol_JHs_occurrence)) {
...
#save copy
#write.csv(dtp_db_v3_1, "C:/dtp_db_v3_1_2.csv", row.names=FALSE)


for (i in 1:nrow(dtp_db_v3_1)) {
  row <- dtp_db_v3_1[i,]
  if (!is.na(row$arrests) & is.na(row$cit_JHs_occurrence)) {
...
#save copy
#write.csv(dtp_db_v3_1, "C:/dbs/dtp_db_v3_1_3.csv", row.names=FALSE)
...
#save copy
#write.csv(dtp_db_v3_1, "C:/dbs/dtp_db_v3_1_4.csv", row.names=FALSE)


for (i in 1:nrow(dtp_db_v3_1)) {
  row <- dtp_db_v3_1[i,]
  if (!is.na(row$arrests) & is.na(row$leoka_assaulted)) {
    row$leoka_assaulted = 0
...
#new version
dtp_db_v4_1 <- dtp_db_v3_1

#save copy
#write.csv(dtp_db_v4_1, "C:/dtp_db_v4_1.csv", row.names=FALSE)
...
#descriptive stats for homicide occurrence
Hmisc::describe(dtp_db_v4_1$homs_occurrence)
stat.desc(dtp_db_v4_1$homs_occurrence)
#percentage of zeroes
sumCol <- dtp_db_v4_1$homs_occurrence
sumCol <- as.data.frame(sumCol)
...
#descriptive stats for justified homicides police
Hmisc::describe(dtp_db_v4_1$pol_JHs_occurrence)
stat.desc(dtp_db_v4_1$pol_JHs_occurrence)
#percentage of zeroes
sumCol <- dtp_db_v4_1$pol_JHs_occurrence
sumCol <- as.data.frame(sumCol)
...
#descriptive stats for justified homicides citizen
Hmisc::describe(dtp_db_v4_1$cit_JHs_occurrence)
stat.desc(dtp_db_v4_1$cit_JHs_occurrence)
#percentage of zeroes
sumCol <- dtp_db_v4_1$cit_JHs_occurrence
sumCol <- as.data.frame(sumCol)
...
#descriptive stats for killed and assaulted officers (LEOKA)
Hmisc::describe(dtp_db_v4_1$leoka_killed)
stat.desc(dtp_db_v4_1$leoka_killed)
#percentage of zeroes
sumCol <- dtp_db_v4_1$leoka_killed
sumCol <- as.data.frame(sumCol)
...
#descriptive stats for assaulted officers (LEOKA)
Hmisc::describe(dtp_db_v4_1$leoka_assaulted)
stat.desc(dtp_db_v4_1$leoka_assaulted)
#percentage of zeroes
sumCol <- dtp_db_v4_1$leoka_assaulted
sumCol <- as.data.frame(sumCol)
...
#read last save because pc turned off on its own volition
dtp_db_v4_1 <- read.csv('C:/dbs/dtp_db_v4_1.csv')
...
dtp_db_v4_1 <- transform(dtp_db_v4_1, k_and_a_leoka=psum(leoka_killed, leoka_assaulted, na.rm=TRUE)) #sum vars
dtp_db_v4_1$k_and_a_leoka[dtp_db_v4_1$k_and_a_leoka == 0] <- NA #transform 0s into NA

# First we separate vars w missing data, do mice, then bring it back
forest_db <- subset(dtp_db_v4_1, select = c(BREAKln_ymo2, hs_grad_pop_interpolation, population_interpolation,
...
#join back again
dtp_db_v4_1_1 <- left_join(dtp_db_v4_1, mice_imp, by = "BREAKln_ymo2")


#save copy
write.csv(dtp_db_v4_1_1, "C:/dtp_db_v4_1_1.csv", row.names=FALSE)

#===============================================================================
# Once again we get the amount of missing data for variables ===================

#descriptive stats for high school graduates population
#Hmisc::describe(dtp_db_v4_1_1$hs_grad_pop_interpolation_imp)
stat.desc(dtp_db_v4_1_1$hs_grad_pop_interpolation_imp)
#percentage of zeroes
sumCol <- dtp_db_v4_1_1$hs_grad_pop_interpolation_imp
sumCol <- as.data.frame(sumCol)
...
#descriptive stats for population
Hmisc::describe(dtp_db_v4_1_1$population_interpolation_imp)
stat.desc(dtp_db_v4_1_1$population_interpolation_imp)
#percentage of zeroes
sumCol <- dtp_db_v4_1_1$population_interpolation_imp
sumCol <- as.data.frame(sumCol)
...
#descriptive stats for black population
Hmisc::describe(dtp_db_v4_1_1$black_pop_interpolation_imp)
stat.desc(dtp_db_v4_1_1$black_pop_interpolation_imp)
#percentage of zeroes
sumCol <- dtp_db_v4_1_1$black_pop_interpolation_imp
sumCol <- as.data.frame(sumCol)
...
#descriptive stats for poverty rate
Hmisc::describe(dtp_db_v4_1_1$poverty_rate_interpolation_imp)
stat.desc(dtp_db_v4_1_1$poverty_rate_interpolation_imp)
#percentage of zeroes
sumCol <- dtp_db_v4_1_1$poverty_rate_interpolation_imp
sumCol <- as.data.frame(sumCol)
...
#descriptive stats for unemployment rate
Hmisc::describe(dtp_db_v4_1_1$unemp_rate_interpolation_imp)
stat.desc(dtp_db_v4_1_1$unemp_rate_interpolation_imp)
#percentage of zeroes
sumCol <- dtp_db_v4_1_1$unemp_rate_interpolation_imp
sumCol <- as.data.frame(sumCol)
...
#descriptive stats for housing occupancy rate
#Hmisc::describe(dtp_db_v4_1_1$housing_occupancy_interpolation_imp)
stat.desc(dtp_db_v4_1_1$housing_occupancy_interpolation_imp)
#percentage of zeroes
sumCol <- dtp_db_v4_1_1$housing_occupancy_interpolation_imp
sumCol <- as.data.frame(sumCol)
...
#descriptive stats for land area
Hmisc::describe(dtp_db_v4_1_1$land_area_interpolation_imp)
stat.desc(dtp_db_v4_1_1$land_area_interpolation_imp)
#percentage of zeroes
sumCol <- dtp_db_v4_1_1$land_area_interpolation_imp
sumCol <- as.data.frame(sumCol)
...
#descriptive stats for water area
Hmisc::describe(dtp_db_v4_1_1$water_area_interpolation_imp)
stat.desc(dtp_db_v4_1_1$water_area_interpolation_imp)
#percentage of zeroes
sumCol <- dtp_db_v4_1_1$water_area_interpolation_imp
sumCol <- as.data.frame(sumCol)
...
#descriptive stats for homicide occurrence
Hmisc::describe(dtp_db_v4_1_1$homs_occurrence_imp)
stat.desc(dtp_db_v4_1_1$homs_occurrence_imp)
#percentage of zeroes
sumCol <- dtp_db_v4_1_1$homs_occurrence_imp
sumCol <- as.data.frame(sumCol)
...
#descriptive stats for justified homicides police
Hmisc::describe(dtp_db_v4_1_1$pol_JHs_occurrence_imp)
stat.desc(dtp_db_v4_1_1$pol_JHs_occurrence_imp)
#percentage of zeroes
sumCol <- dtp_db_v4_1_1$pol_JHs_occurrence_imp
sumCol <- as.data.frame(sumCol)
...
#descriptive stats for justified homicides citizen
Hmisc::describe(dtp_db_v4_1_1$cit_JHs_occurrence_imp)
stat.desc(dtp_db_v4_1_1$cit_JHs_occurrence_imp)
#percentage of zeroes
sumCol <- dtp_db_v4_1_1$cit_JHs_occurrence_imp
sumCol <- as.data.frame(sumCol)
...
#descriptive stats for arrests
Hmisc::describe(dtp_db_v4_1_1$arrests_imp)
stat.desc(dtp_db_v4_1_1$arrests_imp)
#percentage of zeroes
sumCol <- dtp_db_v4_1_1$arrests_imp
sumCol <- as.data.frame(sumCol)
...
#descriptive stats for total police employees
Hmisc::describe(dtp_db_v4_1_1$total_police_employees_interpolation_imp)
stat.desc(dtp_db_v4_1_1$total_police_employees_interpolation_imp)
#percentage of zeroes
sumCol <- dtp_db_v4_1_1$total_police_employees_interpolation_imp
sumCol <- as.data.frame(sumCol)
...
#descriptive stats for killed and assaulted officers (LEOKA)
Hmisc::describe(dtp_db_v4_1_1$k_and_a_leoka_imp)
stat.desc(dtp_db_v4_1_1$k_and_a_leoka_imp)
#percentage of zeroes
sumCol <- dtp_db_v4_1_1$k_and_a_leoka_imp
sumCol <- as.data.frame(sumCol)
...
dtp_db_v4_1_1$pol_total_victims <- dtp_db_v4_1_1$pol_JHs_occurrence_imp + dtp_db_v4_1_1$pol_additional_victim_count
dtp_db_v4_1_1$cit_total_victims <- dtp_db_v4_1_1$cit_JHs_occurrence_imp + dtp_db_v4_1_1$cit_additional_victim_count
dtp_db_v4_1_1$homs_total_victims <- dtp_db_v4_1_1$homs_occurrence_imp + dtp_db_v4_1_1$homs_additional_victim_count
#===============================================================================
...
dtp_db_v4_1_1$hs_grad_rate <- ((dtp_db_v4_1_1$hs_grad_pop_interpolation_imp / dtp_db_v4_1_1$population_interpolation_imp) * 100)
dtp_db_v4_1_1$pop_density <- ((dtp_db_v4_1_1$population_interpolation_imp / dtp_db_v4_1_1$land_area_interpolation_imp) * 100000)


#creating rates per 100,000 population
dtp_db_v4_1_1$murder_rate <- ifelse (is.na((dtp_db_v4_1_1$homs_total_victims / dtp_db_v4_1_1$population_interpolation_imp) * 100000), NA, (dtp_db_v4_1_1$homs_total_victims / dtp_db_v4_1_1$population_interpolation_imp) * 100000)
dtp_db_v4_1_1$arrest_rate <- ifelse (is.na((dtp_db_v4_1_1$arrests_imp / dtp_db_v4_1_1$population_interpolation_imp) * 100000), NA, (dtp_db_v4_1_1$arrests_imp / dtp_db_v4_1_1$population_interpolation_imp) * 100000)
dtp_db_v4_1_1$police_rate <- ifelse (is.na((dtp_db_v4_1_1$total_police_employees_interpolation_imp / dtp_db_v4_1_1$population_interpolation_imp) * 100000), NA, (dtp_db_v4_1_1$total_police_employees_interpolation_imp / dtp_db_v4_1_1$population_interpolation_imp) * 100000)

#do logs
dtp_db_v4_1_1$logarrests <- log(dtp_db_v4_1_1$arrests_imp + 1)
dtp_db_v4_1_1$logmurder_rate <- log(dtp_db_v4_1_1$murder_rate + 1)
dtp_db_v4_1_1$logarrest_rate <- log(dtp_db_v4_1_1$arrest_rate + 1)
dtp_db_v4_1_1$logpolice_rate <- log(dtp_db_v4_1_1$police_rate + 1)
dtp_db_v4_1_1$log_population <- log(dtp_db_v4_1_1$population_interpolation_imp)
dtp_db_v4_1_1$logpop_density <- log(dtp_db_v4_1_1$pop_density + 1)


#remove dups
dtp_db_v4_1_1 <- dtp_db_v4_1_1 %>% distinct(BREAKln_ymo2, .keep_all = TRUE)
...
#joining with main
dtp_db_v4_2 <- left_join(dtp_db_v4_1_1, FY_db, by = "BREAKln_ymo2")

#move column forward
dtp_db_v4_2 <- dtp_db_v4_2 %>% relocate(FY, .before = year)

#save copy
write.csv(dtp_db_v4_2, "C:/dtp_db_v4_2.csv", row.names=FALSE)
